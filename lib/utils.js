...
const generateRandomColors = (object, KeyName) => {
  ...
  return object[KeyName].map(() => `hsl(${Math.floor(Math.random() * 360)}, 100%, 75%)`);
};
...
const fonts = ["Jaro", "Jersey", "Permanent Marker", "Rock Salt", "Yarndings", "Exo", "anta"];

const getRandomFontClass = () => fonts[Math.floor(Math.random() * fonts.length)];

function cn(...inputs) {
  return twMerge(clsx(inputs));
}

export { generateRandomColors, cn, generateColorsProp, getRandomFontClass, fonts };
